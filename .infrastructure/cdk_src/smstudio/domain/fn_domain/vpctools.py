...
logger = logging.getLogger("vpctools")
...
def get_studio_efs_security_group_ids(
    ...
    """Retrieve the security groups you need for [inbound, outbound] comms with SMStudio EFS filesystem

    Returns
    -------
    inbound : Union[str, None]
        Security Group ID for inbound connection from SMStudio filesystem, or None if could not be found
    outbound : str
        Secrity Group ID for outbound connection to SMStudio filesystem, or None if could nom be found

    Raises
    ------
    ValueError :
        If multiple potential SGs are found for either inbound or outbound connection (suggests duplication
        or otherwise erroneous SMStudio/VPC setup).
    Other :
...
def propose_subnet(vpc_id, new_subnet_prefixlen=26):
    ...
    """
    logger.info(f"Proposing admin subnet for VPC {vpc_id}...")
    # Get VPC info:
    ...
    logger.info(f"Parsing existing CIDRs...")
    ...
    # (Could probably skip this since we just retrieved fresh data, but might help to prevent any weird
    # errors manifesting as harder-to-interpret issues further down)
    ...
    logger.info(f"Calculating remaining vacant ranges...")
